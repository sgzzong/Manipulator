...
class MoveGroupPythonIntefaceTutorial(object):
    ...
    def state(self):
        print("Joint angle : ", self.get_joint_state())
    ...
    def reward(self,target):
        print("==============================reward===============================")
        print("target distance :", math.sqrt(abs((self.get_pose_state()[0]-target[0])**2 + (self.get_pose_state()[1]-target[1])**2 + (self.get_pose_state()[2]-target[2])**2 )))
...
def main():
    ...
    for i in range(3):
        for i in range(4):
            ned2_contoller.action(angle[i])
            ned2_contoller.state()
            ned2_contoller.reward(ned2_contoller.target[i])
            ned2_contoller.target_reset(ned2_contoller.target[i])
        ned2_contoller.reset()
